...
void LcdDirRead(uint32_t DummySize)
{
  ...
  #endif
  LL_GPIO_SetPinMode(TS_SCK_GPIO_Port, TS_SCK_Pin, LL_GPIO_MODE_OUTPUT); /* GPIO mode = output */
  while(DummySize--)
  { /* Dummy pulses */
    HAL_GPIO_WritePin(TS_SCK_GPIO_Port, TS_SCK_Pin, 1 - LCDTS_SPI_DEFSTATE);
    HAL_GPIO_WritePin(TS_SCK_GPIO_Port, TS_SCK_Pin, LCDTS_SPI_DEFSTATE);
  }
  LL_GPIO_SetPinMode(TS_SCK_GPIO_Port, TS_SCK_Pin, LL_GPIO_MODE_ALTERNATE); /* GPIO mode = alternative */
